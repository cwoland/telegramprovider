[![CI](https://github.com/cwoland/telegramprovider/actions/workflows/ci.yml/badge.svg)](https://github.com/cwoland/telegramprovider/actions/workflows/ci.yml)
...
- **Токен живёт в браузере.** `idInstance` и `apiTokenInstance` попадают в `localStorage` и в путь
  URL каждого запроса. Для продакшена так нельзя — нужен прокси-бэкенд, который держит токен у
  себя. Для тестового задания это осознанный компромисс; в сообщениях об ошибках токен
  замаскирован, чтобы не утечь в логи и скриншоты.
- **Одна вкладка на инстанс.** Очередь уведомлений в GREEN-API одна: две открытые вкладки будут
  забирать уведомления друг у друга.
- **История только локальная.** GREEN-API не отдаёт переписку, накопленную до запуска приложения,
  — восстанавливается лишь то, что приложение само видело и сохранило.
- **Учётные данные из `localStorage` не перепроверяются** при перезагрузке. Если инстанс за это
  время разлогинился, ошибка появится при первой отправке или от цикла опроса.
- **Только текст.** Медиа, файлы, группы и реакции вне объёма задания.
